...
import questionary  # 需要先安装: pip install questionary

sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from pics.calculate_hash_custom import ImageHashCalculator, PathURIGenerator
...
def initialize_logger():
    ...
    try:
        ...
        logging.info("[#update_log]✅ 日志系统初始化完成")  # 添加面板标识
        ...
        if params['exclude_paths']:
            for exclude_path in params['exclude_paths']:
                if exclude_path in file_path:
                    logging.info( f'文件在排除路径中 (排除关键词: {exclude_path})')
                    return False
        logging.info( '文件通过所有检查，将进行处理')
        return True
