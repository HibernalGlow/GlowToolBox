...
HASH_FILES_LIST = os.path.expanduser(r"E:\1EHV\hash_files_list.txt")  # 新增：保存所有哈希文件路径的文件
...
# 添加黑名单关键词常量
BLACKLIST_KEYWORDS = ['temp', '画集', '00去图', '00不需要', '[00不需要]', '动画']
...
def decode_zip_filename(name: bytes) -> str:
    ...
    Returns:
        解码后的文件名
    """
    encodings = ['utf-8', 'gbk', 'cp437', 'shift-jis']
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                for info in zf.filelist:
                    # 处理文件名编码
                    filename = decode_zip_filename(info.filename.encode('cp437'))
                ...
                logging.info(f"[#file_ops]开始处理压缩包: {zip_path}  共 {total_files} 个图片文件")
            ...
            # 如果zipfile失败，尝试使用py7zr
            logging.error(f"[#hash_calc]使用zipfile处理失败: {e}，尝试使用py7zr")
            try:
                with py7zr.SevenZipFile(zip_path, 'r') as sz:
                    ...
                    image_files = []
                    for f in allfiles:
                        if any(f.filename.lower().endswith(ext) for ext in 
                              ('.jpg', '.jpeg', '.png', '.webp', '.jxl', '.avif', '.bmp')):
                            image_files.append(f.filename)
                    
                    if not image_files:
                        logging.info(f"[#hash_calc]压缩包内无图片文件: {zip_path}")
                        return {}
                        
                    logging.info(f"[#hash_calc]开始处理压缩包: {zip_path}  共 {len(image_files)} 个图片文件")
                    
                    # 创建临时目录用于py7zr提取文件
                    with tempfile.TemporaryDirectory() as tmpdir:
                        # 一次性提取所有文件
                        sz.extract(path=tmpdir, targets=image_files)
                        ...
                        for img_file in image_files:
                            ...
                            with open(img_path, 'rb') as f:
                                process_args.append((img_file, f.read()))
                        
                        # 使用线程池处理图片
                        processed_count = 0
                        total_files = len(process_args)
                        with ThreadPoolExecutor(max_workers=inner_workers) as executor:
                            futures = list(executor.map(process_image, process_args))
                            for _ in futures:
                                processed_count += 1
                                progress = int((processed_count / total_files) * 100)
                                # 修改7z处理进度到专用进度面板
                ...
                logging.error(f"[#hash_calc]使用py7zr处理压缩包失败: {e}")
                
        return zip_hashes
                        
    except Exception as e:
        logging.error(f"[#hash_calc]处理压缩包失败，跳过: {zip_path}: {e}")
        return {}
