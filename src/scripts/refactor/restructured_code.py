...
similarity_config = {'THRESHOLD': 80, 'RATIO_THRESHOLD': 75, 'PARTIAL_THRESHOLD': 85, 'TOKEN_THRESHOLD': 80, 'LENGTH_DIFF_MAX': 0.3}
TEXTUAL_LAYOUT = {'current_stats': {'ratio': 2, 'title': '📊 总体进度', 'style': 'lightyellow'}, 'current_progress': {'ratio': 2, 'title': '🔄 当前进度', 'style': 'lightcyan'}, 'process': {'ratio': 3, 'title': '📝 处理日志', 'style': 'lightpink'}, 'update': {'ratio': 2, 'title': 'ℹ️ 更新日志', 'style': 'lightblue'}}

config = {'script_name': 'manga_archive_classifier', 'console_enabled': False}
logger, config_info = setup_logger(config)

class MainController:
    """
    主控制器，负责协调各个组件工作和处理命令行参数
    """
    enabled_features = {1, 2, 3, 4}
    # Set default values for similarity_config
    ...
    def main(self):
        if sys.platform == 'win32':
            try:
                ...
                print(f'无法设置控制台编码为UTF-8, 错误: {str(e)}')
        paths, args = self.process_args()
        self.run_classifier(paths, args)
    ...
    def run_classifier(self, paths, args):
        ...
        if args.features:
            ...
            enabled_features = {1, 2, 3, 4}
        self.process_paths(paths, enabled_features=enabled_features, wait_for_confirm=args.wait)

    def process_paths(self, paths, enabled_features=None, similarity_config=None, wait_for_confirm=False):
        """处理输入的路径列表"""
        self.init_TextualLogger()
        ...
        for path in paths:
            ...
            if path:
                try:
                    if sys.platform == 'win32':
                        if FileManager.win32_path_exists(path):
                ...
                if sys.platform == 'win32':
                    if FileManager.win32_path_exists(path):
                        if os.path.isdir(path):
                            FileManager.process_directory(path, enabled_features=enabled_features)
                        elif os.path.isfile(path) and ArchiveValidator.is_archive(path):
                            if 1 in enabled_features:
                                if wait_for_confirm:
                                    logger.info(f'[#current_progress] 📦 处理单个文件: {path}')
                                CategoryClassifier.process_single_file(path)
                                if wait_for_confirm:
...
class ArchiveValidator:
    ...
    def count_images_in_archive(self, archive_path, handler=None):
        ...
        try:
            if self.is_archive_corrupted(archive_path):
                logger.warning(f'[#update] ⚠️ 压缩包已损坏，跳过处理: {archive_path}')
                return -1
            output = self.run_7z_command('l', archive_path, additional_args=['-slt'])
            if not output:
...
class CategoryClassifier:
    ...
    def process_single_file(self, abs_path, handler=None):
        ...
        try:
            ...
            self.create_category_folders(os.path.dirname(abs_path))
            category = self.get_category(abs_path)
            if category == '损坏':
                logger.warning(f'[#update] ⚠️ 压缩包已损坏: {os.path.basename(abs_path)}')
                logger.warning(f'[#process] ❌ 损坏: {os.path.basename(abs_path)}')
                FileManager.move_corrupted_archive(abs_path, os.path.dirname(abs_path))
                return
            self.move_file_to_category(abs_path, category)
    ...
    def get_category(self, path, handler=None):
        ...
        if not ArchiveValidator.is_archive(path):
            return '未分类'
        if ArchiveValidator.is_archive_corrupted(path):
            return '损坏'
        for pattern in CATEGORY_RULES['4. 画集']['patterns']:
            if re.search(pattern, filename, re.IGNORECASE):
                return '4. 画集'
        image_count = ArchiveValidator.count_images_in_archive(path)
        if image_count == -1:
            return '损坏'
...
class SeriesExtractor:
            ...
            for root, dirs, _ in os.walk(directory_path):
                for dir_name in dirs:
                    if dir_name.startswith('[#s]'):
                        ...
                        if self.update_series_folder_name(full_path):
                            updated_count += 1
            if updated_count > 0:
    ...
    def create_series_folders(self, directory_path, archives, handler=None):
        ...
        archives = [f for f in archives if os.path.isfile(f) and ArchiveValidator.is_archive(f)]
        ...
        for dir_path, dir_archives in dir_groups.items():
            ...
            series_groups = self.find_series_groups(dir_archives)
            if series_groups:
    ...
    def find_keyword_based_groups(self, remaining_files, file_keys, processed_files, handler=None):
        ...
        for file_path in remaining_files:
            if file_path in processed_files:
                continue
            keywords = self.extract_keywords(os.path.basename(file_path))
            if len(keywords) >= 1:
    ...
    def find_similar_files(self, current_file, files, file_keys, processed_files, handler=None):
        ...
        for other_file in files:
            if other_file in processed_files or other_file == current_file:
                continue
            if self.is_in_series_folder(other_file):
                continue
            if self.is_essentially_same_file(current_file, other_file):
    ...
    def preprocess_filenames(self, files, handler=None):
        ...
        file_keys = {}
        for file_path in files:
            key = self.get_series_key(os.path.basename(file_path))
    ...
    def is_similar_to_existing_folder(self, dir_path, series_name, handler=None):
        ...
        try:
            ...
            return False
        series_key = self.get_series_key(series_name)
        for folder in existing_folders:
            ...
            for prefix in SERIES_PREFIXES:
                if folder.startswith(prefix):
                    ...
                    break
            if is_series_folder:
                folder_key = self.get_series_key(folder_name, handler)
                if series_key == folder_key:
                    if handler:
                        handler.update_panel('update_log', f"📁 找到相同系列文件夹: '{folder}'")
                    return True
                similarity = TextProcessor.calculate_similarity(series_key, folder_key, handler)
                if similarity >= SIMILARITY_CONFIG['THRESHOLD']:
                    if handler:
                        handler.update_panel('update_log', f"📁 找到相似文件夹: '{folder}'")
                    return True
            else:
                similarity = TextProcessor.calculate_similarity(series_name, folder, handler)
                if similarity >= SIMILARITY_CONFIG['THRESHOLD']:
                    if handler:
                        handler.update_panel('update_log', f"📁 找到相似文件夹: '{folder}'")
                    return True
    ...
    def update_series_folder_name(self, old_path, handler=None):
        ...
        try:
            ...
            new_series_name = self.get_series_key(old_series_name)
    ...
    def extract_keywords(self, filename):
        """从文件名中提取关键词"""
        name = TextProcessor.get_base_filename(filename)
        separators = '[\\s]+'
        keywords = []
        name = re.sub('\\[[^\\]]*\\]|\\([^)]*\\)', ' ', name)
    ...
    def is_in_series_folder(self, file_path):
        ...
        for prefix in SERIES_PREFIXES:
            if parent_name.startswith(prefix):
                ...
                parent_key = self.get_series_key(series_name)
                file_key = self.get_series_key(os.path.basename(file_path))
                return parent_key == file_key
        parent_key = self.get_series_key(parent_name)
        file_key = self.get_series_key(os.path.basename(file_path))
    ...
    def is_essentially_same_file(self, file1, file2):
        ...
        base1 = TextProcessor.normalize_chinese(base1)
        base2 = TextProcessor.normalize_chinese(base2)
        return base1 == base2
    ...
    def get_series_key(self, filename, handler=None):
        ...
        series_groups = self.find_series_groups(test_group)
        if series_groups:
            series_name = next(iter(series_groups.keys()))
            logger.info(f'[#process] 找到系列名称: {series_name}')
            return series_name
        name = self.preprocess_filename(filename)
        name = TextProcessor.normalize_chinese(name)
    ...
    def find_series_groups(self, filenames, handler=None):
        ...
        processed_names = {f: self.preprocess_filename(f) for f in filenames}
        processed_keywords = {f: self.get_keywords(processed_names[f]) for f in filenames}
        simplified_names = {f: TextProcessor.normalize_chinese(n) for f, n in processed_names.items()}
        simplified_keywords = {f: [TextProcessor.normalize_chinese(k) for k in kws] for f, kws in processed_keywords.items()}
        ...
        while remaining_files:
            ...
            for file1 in remaining_files:
                ...
                base_name1 = TextProcessor.get_base_filename(os.path.basename(file1))
                for file2 in remaining_files - {file1}:
                    if file2 in matched_files:
                        continue
                    base_name2 = TextProcessor.get_base_filename(os.path.basename(file2))
                    ...
                    common = self.find_longest_common_keywords(keywords1, keywords2)
                    if common:
                        original_kw1 = processed_keywords[file1]
                        original_common = original_kw1[keywords1.index(common[0]):keywords1.index(common[-1]) + 1]
                        series_name = self.validate_series_name(' '.join(original_common))
            ...
            if best_pair and best_series_name:
                ...
                base_name1 = TextProcessor.get_base_filename(os.path.basename(best_pair[0]))
                for other_file in remaining_files - matched_files_this_round - matched_files:
                    other_base_name = TextProcessor.get_base_filename(os.path.basename(other_file))
                    if base_name1 == other_base_name:
                        continue
                    other_keywords = simplified_keywords[other_file]
                    common = self.find_longest_common_keywords(simplified_keywords[best_pair[0]], other_keywords)
        ...
        if remaining_files:
            ...
            for file in list(remaining_files):
                ...
                for series_name in existing_series:
                    series_base = TextProcessor.normalize_chinese(series_name)
                    series_base_no_space = re.sub('\\s+', '', series_base)
                    if series_base_no_space in base_name_no_space:
                        base_name_current = TextProcessor.get_base_filename(os.path.basename(file))
                        has_same_base = False
                        for existing_file in matched_files_by_series[series_name]:
                            if TextProcessor.get_base_filename(os.path.basename(existing_file)) == base_name_current:
                                has_same_base = True
                                break
                ...
                if best_pair and best_common and (len(best_common.strip()) > 1):
                    ...
                    for other_file in remaining_files - matched_files_this_round - matched_files:
                        ...
                        if best_common in other_base:
                            matched_files_this_round.add(other_file)
                    series_name = self.validate_series_name(original_form)
                    if series_name:
...
class FileManager:
    ...
    def process_directory(self, directory_path, progress_task=None, enabled_features=None, handler=None):
        ...
        try:
            if enabled_features is None:
                enabled_features = {1, 2, 3, 4}
            abs_dir_path = self.validate_directory(directory_path)
            if not abs_dir_path:
                return []
            MainController.init_TextualLogger()
            try:
                logger.info(f'[#process] 📂 开始处理目录: {abs_dir_path}')
                if 2 in enabled_features:
                    logger.info('[#process] 🔄 检查并更新旧的系列文件夹名称...')
                    SeriesExtractor.update_all_series_folders(abs_dir_path)
                if 1 in enabled_features:
                    CategoryClassifier.create_category_folders(abs_dir_path)
                category_folders = set(CATEGORY_RULES.keys())
                found_archives = False
                if 2 in enabled_features:
                    logger.info('[#process] 🔍 开始查找可提取系列的压缩包...')
                    archives = self.collect_archives_for_series(abs_dir_path, category_folders)
                    if archives:
                        found_archives = True
                        total_archives = len(archives)
                        logger.info(f"[#update] ✨ 在目录 '{abs_dir_path}' 及其子文件夹下找到 {total_archives} 个有效压缩包")
                        SeriesExtractor.create_series_folders(abs_dir_path, archives)
                        logger.info('[#current_progress] 系列提取完成')
                    else:
                        logger.info('[#process] 没有找到可提取系列的压缩包')
                if 1 in enabled_features:
                    logger.info('[#process] 🔍 开始查找需要分类的压缩包...')
                    archives = self.collect_archives_for_category(abs_dir_path, category_folders)
                    if archives:
                        found_archives = True
                        total_archives = len(archives)
    ...
    def validate_directory(self, directory_path, handler=None):
        ...
        if not os.path.isdir(abs_dir_path):
            ...
            return None
        if CategoryClassifier.is_path_blacklisted(abs_dir_path):
    ...
    def collect_archives_for_series(self, directory_path, category_folders, handler=None):
        ...
        for root, _, files in os.walk(directory_path):
            ...
            if current_level - base_level > 1:
                continue
            if CategoryClassifier.is_path_blacklisted(root):
            ...
            if current_dir.startswith('[#s]') or current_dir == '损坏压缩包':
                continue
            for file in files:
                if ArchiveValidator.is_archive(file):
                    file_path = os.path.join(root, file)
                    if SeriesExtractor.is_series_blacklisted(file):
                        logger.warning(f'[#update] ⚠️ 文件在系列提取黑名单中，跳过: {file}')
                        continue
                    if CategoryClassifier.is_path_blacklisted(file):
                ...
                for i, future in enumerate(futures, 1):
                    ...
                    try:
                        ...
                        if is_corrupted:
                            ...
                            self.move_corrupted_archive(path, directory_path, handler)
                        else:
    ...
    def collect_archives_for_category(self, directory_path, category_folders, handler=None):
        ...
        with os.scandir(directory_path) as entries:
            for entry in entries:
                if entry.is_file() and ArchiveValidator.is_archive(entry.name):
...
class TextProcessor:
    ...
    def calculate_similarity(self, str1, str2, handler=None):
        """计算两个字符串的相似度"""
        str1 = self.normalize_chinese(str1)
        str2 = self.normalize_chinese(str2)
    ...
    def get_base_filename(self, filename):
        ...
        name = re.sub('\\([^)]*\\)', '', name)
        name = re.sub('[\\s!！?？_~～]+', '', name)
        name = self.normalize_chinese(name)
        return name
...
PRESET_CONFIGS = {
    "标准分类": {
        "description": "标准的漫画分类配置，包含所有基础分类功能",
        "checkbox_options": [
            "enable_category",  # 启用分类功能
            "enable_series",    # 启用系列检测
            "enable_validate"   # 启用压缩包验证
        ],
        "input_values": {
            "similarity": "75",      # 相似度阈值
            "min_images": "100",     # 最小图片数量
            "max_length": "0.3"      # 最大长度差异
        }
    },
    "快速分类": {
        "description": "快速分类模式，仅进行基础分类，不进行详细验证",
        "checkbox_options": [
            "enable_category"    # 只启用分类功能
        ],
        "input_values": {
            "similarity": "60",      # 降低相似度要求
            "min_images": "50",      # 降低最小图片数要求
            "max_length": "0.5"      # 增加长度差异容忍度
        }
    },
    "严格模式": {
        "description": "严格的分类模式，进行全面的验证和分类",
        "checkbox_options": [
            "enable_category",
            "enable_series",
            "enable_validate",
            "enable_strict"      # 启用严格模式
        ],
        "input_values": {
            "similarity": "85",      # 提高相似度要求
            "min_images": "150",     # 提高最小图片数要求
            "max_length": "0.2"      # 降低长度差异容忍度
        }
    },
    "仅系列检测": {
        "description": "只进行系列检测，不进行分类",
        "checkbox_options": [
            "enable_series"
        ],
        "input_values": {
    ...
    "仅压缩包验证": {
        "description": "只验证压缩包完整性，不进行分类和系列检测",
        "checkbox_options": [
            "enable_validate"
        ],
        "input_values": {}
