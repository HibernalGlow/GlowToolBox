        ...
        try:
            ...
            try:
                with zipfile.ZipFile(archive_path, 'a') as zf:
                    for file in files_to_delete:
                        try:
                            zf.remove(file)
                            logger.info(f"[#process]删除文件(方法1): {file}")
                        except KeyError:
                            pass
                success = True
            except Exception as e:
                logger.debug(f"[#process]方法1失败: {str(e)}")
                ...
                try:
                    ...
                    for pattern in patterns:
                        try:
                            ...
                            logger.info(f"[#process]删除文件(方法2-通配符): {pattern}")
                    ...
                    for file in files_to_delete:
                        try:
                            ...
                            logger.info(f"[#process]删除文件(方法2-具体): {file}")
                        except subprocess.CalledProcessError:
                            continue
                    success = True
                except Exception as e:
                    logger.debug(f"[#process]方法2失败: {str(e)}")
            ...
            if not success:
                ...
                try:
                    ...
                    success = True
                    logger.info(f"[#process]删除文件(方法3-批量): 完成")
                except Exception as e:
                    logger.debug(f"[#process]方法3失败: {str(e)}")

            return success

        except Exception as e:
            logger.error(f"[#process]所有删除方法都失败: {str(e)}")
            return False
        finally:
            # 清理临时文件
