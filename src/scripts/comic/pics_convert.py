...
def init_layout():
    ...
    logging.info(f"[#performance]初始化性能配置面板")
    logging.info(f"[#file_ops]初始化文件操作面板")
    logging.info(f"[#archive_ops]初始化压缩包处理面板")
...
class FileSystem:
    ...
    def ensure_directory_exists(self, directory):
        ...
        try:
            if not self.fs.exists(directory):
                ...
                logging.info(f"[#file_ops]创建目录: {directory}")
            return True
        except Exception as e:
            logging.info(f"[#file_ops]创建目录失败 {directory}: {e}")
            return False
    ...
    def safe_delete_file(self, file_path):
        ...
        try:
            if self.fs.exists(file_path):
                ...
                logging.info(f"[#file_ops]删除文件: {file_path}")
                return True
            return False
        except Exception as e:
            logging.info(f"[#file_ops]删除文件失败 {file_path}: {e}")
            return False
    ...
    def safe_move_file(self, src_path, dst_path):
        ...
        try:
            if not self.fs.exists(src_path):
                logging.info(f"[#file_ops]源文件不存在: {src_path}")
                return False
            if self.fs.exists(dst_path):
                logging.info(f"[#file_ops]目标文件已存在: {dst_path}")
                return False
            self.fs.move(src_path, dst_path)
            logging.info(f"[#file_ops]移动文件: {src_path} -> {dst_path}")
            return True
        except Exception as e:
            logging.info(f"[#file_ops]移动文件失败 {src_path} -> {dst_path}: {e}")
            return False
    ...
    def safe_copy_file(self, src_path, dst_path):
        ...
        try:
            ...
            logging.info(f"[#file_ops]复制文件: {src_path} -> {dst_path}")
            return True
        except Exception as e:
            logging.info(f"[#file_ops]复制文件失败 {src_path} -> {dst_path}: {e}")
            return False
    ...
    def get_file_size(self, file_path):
        ...
        try:
            ...
            logging.info(f"[#file_ops]获取文件大小失败 {file_path}: {e}")
            return 0
    ...
    def list_files(self, directory, pattern=None):
        ...
        try:
            ...
            logging.info(f"[#file_ops]列出文件失败 {directory}: {e}")
            return []
...
class PathHandler:
    ...
    def ensure_long_path(path):
        ...
        try:
            ...
            logging.info(f"[#file_ops]处理长路径时出错: {e}")
            return Path(path)
    ...
    def create_temp_directory(self, file_path):
        ...
        try:
            ...
            logging.info(f"[#file_ops]创建临时目录: {safe_temp_dir}")  
            return safe_temp_dir
        except Exception as e:
            logging.info(f"[#file_ops]创建临时目录失败: {e}")
            raise
    ...
    def wrapper(self, path, *args, **kwargs):
        ...
        try:
            ...
            if e.winerror == 3:
                logging.info(f"[#file_ops]路径超长或无效: {path}")
            else:
                raise
    ...
    def flatten_single_subfolder(self, directory, exclude_keywords):
        ...
        try:
            ...
            for item in os.listdir(subdir):
                ...
                shutil.move(src, dst)
            os.rmdir(subdir)
            logging.info(f"[#file_ops]已展平子文件夹: {subdir}")
        except Exception as e:
            logging.info(f"[#file_ops]展平子文件夹时出错: {e}")
            ...
            for dir_name in dirs:
                ...
                try:
                    if not os.listdir(dir_path):
                        ...
                        removed_count += 1
                        logging.info(f"[#file_ops]已删除空文件夹: {dir_path}")
                except Exception as e:
                    logging.info(f"[#file_ops]删除空文件夹失败 {dir_path}: {e}")
...
class Converter:
    ...
    def convert_with_cjxl(self, input_path, output_path, is_jpeg=False):
        ...
        try:
            if not CJXL_PATH.exists():
                logging.info(f"[#file_ops]cjxl.exe不存在: {CJXL_PATH}")
                return False
            ...
            if result.returncode == 0:
                logging.info(f"[#file_ops]cjxl转换成功: {input_path}")
                return True
            else:
                logging.info(f"[#file_ops]cjxl转换失败: {input_path}\n错误: {result.stderr}")
                return False
                
        except Exception as e:
            logging.info(f"[#file_ops]cjxl转换出错: {e}")
            return False
    ...
    def process_single_image(self, file_path, params):
        ...
        try:
            ...
            if target_ext == '.jxl' and params.get('use_cjxl', False):
                ...
                logging.info(f"[#image_convert]✅ 使用cjxl转换: {file_path}")
                ...
                # logging.info(f"[#image_convert]✅ 使用libvip转换: {file_path}")
            if fs.exists(new_file_path):
                ...
                # logging.info(f"[#file_ops]{status_message}")
                try:
                    image = None
                    fs.delete(file_path)
                    ...
                    logging.info(f"[#image_convert]{error_msg}")
                    return False
            return False
        except Exception as e:
        ...
        try:
            with BytesIO(image_data) as bio:
                with Image.open(bio) as img:
                    ...
                    logging.info(f"[#image_convert]处理图片: 格式={original_format}, 尺寸={original_dimensions}{width_info}, 大小={original_size:.2f}KB")
                    if min_width > 0:
                        if img.width < min_width:
                            logging.info(f"[#image_convert][宽度过小] 图片宽度 {img.width}px 小于指定的最小宽度 {min_width}px，跳过处理")
                            return (image_data, 'width_too_small')
                        else:
                            logging.info(f"[#image_convert][宽度符合] 图片宽度 {img.width}px 大于指定的最小宽度 {min_width}px，继续处理")
                    cur_format = img.format.lower()
                    target_format = IMAGE_CONVERSION_CONFIG['target_format'][1:].lower()
                    if cur_format == target_format:
                        logging.info(f"[#image_convert]图片已经是目标格式 {target_format}，跳过转换")
                        return (image_data, None)
            image = pyvips.Image.new_from_buffer(image_data, '')
            config = IMAGE_CONVERSION_CONFIG[f'{target_format}_config']
            logging.info(f"[#image_convert]转换配置: 目标格式={target_format}, 参数={config}")
            ...
            logging.info(f"[#image_convert]转换完成: 新大小={converted_size:.2f}KB, 减少={size_change:.2f}KB ({size_change / original_size * 100:.1f}%)")
            return (output_buffer, None)
        except Exception as e:
            logging.info(f"[#image_convert]图片转换错误: {str(e)}")
    ...
    def has_processed_comment(self, zip_path, comment='Processed'):
        try:
            ...
            logging.info(f"[#file_ops]Error checking comment in {zip_path}: {e}")
            return False
    ...
    def add_processed_comment(self, zip_path, comment='Processed'):
        try:
            ...
            logging.info(f"[#archive_ops]Added comment '{comment}' to {zip_path}")
        except Exception as e:
            logging.info(f"[#archive_ops]Error adding comment to {zip_path}: {e}")
...
class BatchProcessor:
    ...
    def _write_log_header(self, log_file, initial_count, archive_path):
        ...
        # 同时更新到面板
        logging.info(f"[#archive_ops]📝 开始处理压缩包: {os.path.basename(archive_path) if archive_path else '未知'}")
    def _write_conversion_params(self, log_file):
        ...
        logging.info(f"[#image_convert]{params_text}")
    ...
    def _write_log_summary(self, log_file, processed_files, total_time, total_original_size, total_converted_size):
        ...
        logging.info(f"[#archive_ops]{summary_text}")
    ...
    def _process_image_batch(self, batch, params, processed_files, log_file_path, temp_dir, total_status):
        ...
        logging.info(f"[#performance]当前线程数: {current_threads}, 当前批处理大小: {batch_size}")
            ...
            for future, file_path in futures:
                try:
                    ...
                    if isinstance(result, tuple) and result[0]:
                        ...
                        logging.info(f"[#image_convert]✅ {message}")
                        ...
                        logging.info(f"[@progress] 当前进度: {archive_ratio} {archive_status:.1f}%")
                    ...
                    logging.info(f"[#file_ops]❌ 处理图片失败 {os.path.relpath(file_path, temp_dir)}: {e}")
                    with open(log_file_path, 'a', encoding='utf-8') as f:
    ...
    def process_images_in_directory(self, temp_dir, params, archive_path=None):
        ...
        try:
            ...
            if not image_files:
                logging.info(f"[#file_ops]未找到图片文件在目录: {temp_dir}")
                return set()
            ...
            logging.info(f"[#file_ops]处理目录的图片时出错: {e}")
            return set()
...
class ArchiveHandler:
    ...
    def _validate_archive(self, file_path: Path, params: dict) -> tuple[bool, int]:
        ...
        if not self.should_process_file(file_path, params):
            logging.info(f"[#archive_ops]根据过滤条件跳过文件: {file_path}")
            logging.info(f"[#archive_ops]跳过: {file_path.name} - 不符合关键词要求")
            return False, 0
            
        logging.info(f"[#archive_ops]🔄 正在处理: {file_path.name}")
            
        needs_processing, image_count = self.check_archive_contents(str(file_path), params.get('min_width', 0))
        
        if needs_processing is None:
            logging.info(f"[#archive_ops]文件被占用，将添加到重试队列: {file_path}")
            return False, 0
        elif needs_processing is False:
            logging.info(f"[#archive_ops]压缩包 {file_path} 无需处理")
            return False, 0
        elif image_count == 0:
            logging.info(f"[#archive_ops]压缩包 {file_path} 不包含图片文件")
            return False, 0
    ...
    def _prepare_paths(self, file_path: Path) -> tuple[Path, Path, Path]:
        ...
        logging.info(f"[#file_ops]创建备份: {backup_file_path}")
    ...
    def _process_archive_contents(self, file_path: Path, temp_dir: Path, params: dict, 
        ...
        if not self.extract_archive(file_path, temp_dir):
            logging.info(f"[#file_ops]解压失败: {file_path}")
            return set(), {}
            
        logging.info(f"[#image_convert]正在处理图片: {file_path.name}")
    ...
    def _finalize_archive(self, file_path: Path, temp_dir: Path, new_zip_path: Path,
        ...
        if any((reason == '连续低效率转换' for reason in skipped_files.values())):
            logging.info(f"[#archive_ops]压缩包 {file_path} 因连续低效率转换被跳过")
            logging.info(f"[#archive_ops]跳过: {file_path.name} - 连续低效率转换")
            return []
            
        if not processed_files:
            logging.info(f"[#archive_ops]没有需要处理的图片: {file_path}")
            return []
            
        logging.info(f"[#archive_ops]正在创建新压缩包: {file_path.name}")
            
        if not ArchiveContent().cleanup_and_compress(temp_dir, processed_files, skipped_files, new_zip_path):
            logging.info(f"[#archive_ops]清理和压缩失败: {file_path}")
            logging.info(f"[#archive_ops]错误: {file_path.name} - 清理和压缩失败")
            return []
        ...
        if success:
            ...
            logging.info(f"[#archive_ops]完成: {file_path.name} - 减少了 {size_change:.2f}MB")
        else:
            logging.info(f"[#archive_ops]跳过: {file_path.name} - 新文件大小未减小")
    ...
    def process_single_archive(self, file_path, params):
        ...
        try:
            ...
            logging.info(f"[#file_ops]开始处理文件: {file_path}")
            ...
            logging.info(f"[#archive_ops]处理压缩包时出错 {file_path}: {e}")
            logging.info(f"[#archive_ops]错误: {file_path.name} - {str(e)}")
            return []
    ...
    def run_7z_command(self, command, zip_path, operation='', additional_args=None):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode == 0:
                logging.info(f"[#file_ops]成功执行7z {operation}: {zip_path}")
                return (True, result.stdout)
            else:
                logging.info(f"[#file_ops]7z {operation}失败: {zip_path}\n错误: {result.stderr}")
                return (False, result.stderr)
        except Exception as e:
            logging.info(f"[#file_ops]执行7z命令出错: {e}")
            return (False, str(e))
    ...
    def create_new_archive(self, temp_dir, new_zip_path):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode != 0:
                logging.info(f"[#file_ops]创建压缩包失败: {safe_zip}\n错误: {result.stderr}")
                return False
            fs = fsspec.filesystem('file')
            if not fs.exists(str(safe_zip)):
                logging.info(f"[#file_ops]压缩包创建失败，文件不存在: {safe_zip}")
                return False
            logging.info(f"[#file_ops]成功创建新压缩包: {safe_zip}")
            return True
        except Exception as e:
            logging.info(f"[#file_ops]创建压缩包时出错: {e}")
            return False
    ...
    def check_archive_contents(self, file_path, min_width=0):
        ...
        try:
            try:
                ...
                logging.info(f"[#file_ops]文件正在被占用，稍后重试: {file_path}")
                return (None, 0)
            with zipfile.ZipFile(file_path, 'r') as zip_ref:
                file_list = zip_ref.namelist()
                if not file_list:
                    logging.info(f"[#file_ops]压缩包为空: {file_path}")
                    return (False, 0)
                target_ext = IMAGE_CONVERSION_CONFIG['target_format'].lower()
                image_count = 0
                ...
                try:
                    for file_name in file_list:
                        ...
                        if file_ext in VIDEO_FORMATS:
                            has_video = True
                            logging.info(f"[#file_ops]发现视频文件: {file_name}")
                            return (False, 0)
                        elif file_ext in AUDIO_FORMATS:
                            has_audio = True
                            logging.info(f"[#file_ops]发现音频文件: {file_name}")
                            return (False, 0)
                        elif file_ext in EXCLUDED_IMAGE_FORMATS:
                            has_excluded_format = True
                            logging.info(f"[#file_ops]发现排除格式图片: {file_name}")
                            return (False, 0)
                                ...
                                if min_width > 0 and temp_dir:
                                    try:
                                        ...
                                        with Image.open(img_path) as img:
                                            if img.width < min_width:
                                                logging.info(f"[#image_convert]发现宽度不足的图片: {file_name} (宽度: {img.width}px < {min_width}px)")
                                                return (False, 0)
                                    except Exception as e:
                                        logging.info(f"[#file_ops]检查图片宽度时出错 {file_name}: {e}")
                                        continue
                                    finally:
                                        try:
                    ...
                    if temp_dir and os.path.exists(temp_dir):
                        try:
                            ...
                            pass
                if needs_processing:
                    logging.info(f"[#file_ops]压缩包 {file_path} 包含 {image_count} 个图片文件，需要处理")
                return (needs_processing, image_count)
        except zipfile.BadZipFile:
            logging.info(f"[#file_ops]无效的压缩包格式: {file_path}")
            return (False, 0)
        except Exception as e:
            logging.info(f"[#file_ops]检查压缩包内容时出错 {file_path}: {e}")
    ...
    def should_process_file(self, file_path, params):
        ...
        if params.get('exclude_paths'):
            ...
            if is_excluded:
                logging.info(f"[#file_ops]文件在排除路径中，跳过: {file_path}")
                return False
        if params.get('keywords'):
            ...
            if params['keywords'] == 'internal':
                ...
                if not has_keyword:
                    logging.info(f"[#file_ops]文件名不包含内置关键词，跳过: {file_path}")
                    return False
                logging.info(f"[#file_ops]文件名包含内置关键词，继续处理: {file_path}")
            elif isinstance(params['keywords'], list):
                has_keyword = any((keyword.lower() in file_name for keyword in params['keywords']))
                if not has_keyword:
                    logging.info(f"[#file_ops]文件名不包含指定关键词，跳过: {file_path}")
                    return False
                logging.info(f"[#file_ops]文件名包含指定关键词，继续处理: {file_path}")
    ...
    def extract_archive(self, file_path, temp_dir):
        ...
        try:
            ...
            logging.info(f"[#file_ops]开始解压: {safe_src} ")
            try:
                # 使用 7z x 命令，保持目录结构
                cmd = ['7z', 'x', str(safe_src), f'-o{str(safe_dest)}', '-y']
                result = subprocess.run(cmd, capture_output=True, text=True)
                if result.returncode == 0:
                    logging.info(f"[#file_ops]使用7z成功解压: {safe_src}")
                    return True
                else:
                    logging.info(f"[#file_ops]7z解压失败，尝试备用方案: {result.stderr}")
                    
                # 如果 x 命令失败，尝试使用 e 命令（不保持目录结构）
                # cmd = ['7z', 'e', str(safe_src), f'-o{str(safe_dest)}', '-y']
                # result = subprocess.run(cmd, capture_output=True, text=True)
                # if result.returncode == 0:
                #     logging.info(f"[#file_ops]使用7z (e)成功解压: {safe_src}")
                #     return True
                # else:
                #     logging.info(f"[#file_ops]7z (e)解压也失败，尝试其他方案: {result.stderr}")
            except Exception as e:
                logging.info(f"[#file_ops]7z解压出错，尝试备用方案: {e}")
            ...
            try:
                with zipfile.ZipFile(str(safe_src), 'r') as zip_ref:
                    file_list = zip_ref.namelist()
                    if not file_list:
                        logging.info(f"[#file_ops]压缩包为空: {safe_src}")
                        return False
                    for file_name in file_list:
                    ...
                    logging.info(f"[#file_ops]使用zipfile成功解压: {safe_src}")
                    return True
            except zipfile.BadZipFile:
                logging.info(f"[#file_ops]zipfile解压失败: {safe_src}")
            except Exception as e:
                logging.info(f"[#file_ops]zipfile解压出错: {e}")
                
            logging.info(f"[#file_ops]所有解压方案都失败: {safe_src}")
            return False
        except Exception as e:
            logging.info(f"[#file_ops]解压文件时出错: {e}")
            return False
    ...
    def handle_size_comparison(self, file_path, new_zip_path, backup_file_path):
        ...
        try:
            ...
            if not fs.exists(str(safe_new)):
                logging.info(f"[#file_ops]新压缩包不存在: {safe_new}")
                return (False, 0)
            ...
            if new_size >= original_size * SIZE_THRESHOLD_RATIO:
                logging.info(f"[#file_ops]新压缩包 ({new_size / 1024 / 1024:.2f}MB) 大小超过原始文件 ({original_size / 1024 / 1024:.2f}MB) 的{SIZE_THRESHOLD_RATIO*100}%，压缩效果不理想")
                fs.delete(str(safe_new))
                if fs.exists(str(safe_backup)):
                    ...
                    if hasattr(self, 'rename_cbr') and self.rename_cbr:
                        ...
                        logging.info(f"[#file_ops]已将文件改为CBR: {new_name}")
                return (False, 0)
            try:
                with fs.open(str(safe_file), 'rb') as f:
                    f.read(1)
            except Exception as e:
                logging.info(f"[#file_ops]无法访问目标文件，可能正在被使用: {safe_file}")
                return (False, 0)
            try:
                fs.delete(str(safe_file))
                fs.move(str(safe_new), str(safe_file))
            except Exception as e:
                logging.info(f"[#file_ops]替换文件时出错: {e}")
                if fs.exists(str(safe_backup)):
                    try:
                        fs.move(str(safe_backup), str(safe_file))
                        logging.info("[#file_ops]已还原原始文件")
                    except Exception as restore_error:
                        logging.info(f"[#file_ops]还原文件失败: {restore_error}")
                return (False, 0)
            if fs.exists(str(safe_backup)):
                try:
                    fs.delete(str(safe_backup))
                    ...
                    logging.info(f"[#file_ops]删除备份文件失败: {e}")
            size_change = (original_size - new_size) / (1024 * 1024)
            logging.info(f"[#file_ops]更新压缩包: {safe_file} (减少 {size_change:.2f}MB)")
            return (True, size_change)
        except Exception as e:
            logging.info(f"[#file_ops]比较文件大小时出错: {e}")
            if fs.exists(str(safe_backup)):
                try:
                    fs.move(str(safe_backup), str(safe_file))
                    logging.info("[#file_ops]已还原原始文件")
            ...
            if fs.exists(str(safe_new)):
                try:
                    ...
                    logging.info(f"[#file_ops]删除新文件失败: {remove_error}")
            return (False, 0)
...
class ArchiveContent:
    ...
    def cleanup_and_compress(self, temp_dir, processed_files, skipped_files, new_zip_path):
        """清理文件并创建新压缩包"""
        try:
            logging.info(f"[#file_ops]处理了 {len(processed_files)} 张图片，跳过了 {len(skipped_files)} 张图片")
            ...
            for file_path in processed_files:
                try:
                    if file_path and os.path.exists(file_path):
                        os.remove(file_path)
                        removed_count += 1
                        logging.info(f"[#file_ops]已删除文件: {file_path}")
                except Exception as e:
                    logging.info(f"[#file_ops]删除文件失败 {file_path}: {e}")
                    continue
            logging.info(f"[#file_ops]已删除 {removed_count} 个文件")
            empty_dirs_removed = self.directory_handler.remove_empty_directories(temp_dir)
            if empty_dirs_removed != 0:
                logging.info(f"[#file_ops]已删除 {empty_dirs_removed} 个空文件夹")
            self.directory_handler.flatten_single_subfolder(temp_dir, [])
            if not os.path.exists(temp_dir):
                logging.info(f"[#file_ops]临时目录不存在: {temp_dir}")
                return False
            if not any(os.scandir(temp_dir)):
                logging.info(f"[#file_ops]临时目录为空: {temp_dir}")
                return False
            try:
                ...
                if not os.path.exists(new_zip_path):
                    logging.info(f"[#archive_ops]压缩包创建失败: {new_zip_path}")
                    return False
                logging.info(f"[#archive_ops]成功创建新压缩包: {new_zip_path}")
                return True
            except Exception as e:
                logging.info(f"[#archive_ops]创建压缩包失败: {e}")
                return False
        except Exception as e:
            logging.info(f"[#archive_ops]清理和压缩时出错: {e}")
            return False
        ...
        try:
            ...
            if result.returncode == 0:
                if not os.path.exists(zip_path):
                    logging.info(f"[#archive_ops]压缩包创建失败: {zip_path}")
                    return False
                logging.info(f"[#archive_ops]成功创建新压缩包: {zip_path} ({os.path.getsize(zip_path) / 1024 / 1024:.2f} MB)")
                return True
            else:
                logging.info(f"[#archive_ops]创建压缩包失败: {result.stderr}")
                return False
        except Exception as e:
            logging.info(f"[#file_ops]创建压缩包时出错: {e}")
            return False
    ...
    def read_zip_contents(self, zip_path):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode != 0:
                logging.info(f"[#file_ops]读取压缩包失败: {zip_path}\n错误: {result.stderr}")
                return []
            files = []
            cur_file = None
            ...
            logging.info(f"[#file_ops]Found {len(files)} files in archive: {zip_path}")
            return files
        except Exception as e:
            logging.info(f"[#file_ops]读取压缩包内容时出错 {zip_path}: {e}")
            return []
    ...
    def delete_backup_if_successful(self, backup_path):
        ...
        if os.path.exists(backup_path) and backup_path.endswith('.bak'):
            try:
                logging.info(f"[#file_ops]将备份文件移至回收站: {backup_path}")
                send2trash(backup_path)
            except Exception as e:
                logging.info(f"[#file_ops]移动备份文件到回收站失败: {backup_path} - {e}")
    ...
    def backup_removed_files(self, zip_path, removed_files, duplicate_files):
        ...
        try:
            ...
            for file_path in duplicate_files:
                ...
                os.makedirs(os.path.dirname(dest_path), exist_ok=True)
                shutil.copy2(file_path, dest_path)
            logging.info(f"[#file_ops]已备份删除的文件到: {trash_dir}")
        except Exception as e:
            logging.info(f"[#file_ops]备份删除文件时出错: {e}")
...
class PerformanceConfig:
    ...
    def _load_config(self):
        ...
        try:
            ...
            logging.info(f"[#file_ops]加载性能配置文件失败: {e}")
    ...
    def set_performance_config_path(self, path):
        ...
        if os.path.exists(path):
            ...
            if success:
                logging.info(f"[#file_ops]已设置性能配置文件路径: {path}")
            return success
        else:
            logging.info(f"[#file_ops]性能配置文件不存在: {path}")
            return False
...
class Monitor:
    ...
    def handle_config_update(self, new_threads: int, new_batch: int):
        ...
        logging.info(f"[#config] 应用新配置: 线程数={new_threads} 批处理={new_batch}")
        ...
        logging.info(f"[#runtime] 配置已生效 | 线程: {new_threads} | 批次: {new_batch}")
    def _update_executor(self, new_threads: int, new_batch: int):
        """动态更新线程池的实例方法"""
        global executor
        executor.shutdown(wait=False)
        executor = ThreadPoolExecutor(max_workers=new_threads)
        logging.info(f"[#performance] 线程池已更新至{new_threads} workers")
    ...
    def update_performance_info():
        ...
        batch_size = get_batch_size()
        logging.info(f"[#performance]线程数: {thread_count} 批处理大小: {batch_size} ")
    ...
    def auto_run_process(self, directories, params, interval_minutes=10, infinite_mode=False):
        ...
        try:
            ...
            logging.info("[#file_ops]⚠️ 用户中断处理")
        except Exception as e:
            logging.info(f"[#file_ops]❌ 处理过程出错: {e}")
    ...
    def _update_status(self):
        ...
        if self.total_files > 0:
            ...
            logging.info(f"[#status]总进度: (✅{self.processed_files}+⏭️{self.skipped_files}/{self.total_files}) {progress:.1f}%")
    ...
    def _run_process_loop(self, directories, params, interval_minutes, infinite_mode):
        ...
        while True:
            try:
                round_count += 1
                logging.info(f"[#file_ops]🔄 开始第 {round_count} 轮处理...")
                ...
                if not files_to_process:
                    if infinite_mode:
                        logging.info("[#file_ops]⏸️ 当前没有需要处理的文件，继续监控...")
                        self._wait_next_round(interval_minutes)
                        continue
                    else:
                        logging.info("[#file_ops]✅ 所有文件已处理完成")
                        break
                ...
                if infinite_mode or occupied_files or len(processed_files) > 0 or len(skipped_files) > 0:
                    ...
                    logging.info(f"[#file_ops]⏸️ 等待 {wait_minutes} 分钟后开始下一轮...")
                ...
                logging.info(f"[#file_ops]❌ 处理过程出错: {e}")
                if infinite_mode:
                    logging.info(f"[#file_ops]⚠️ 处理出错: {e}，等待下一轮...")
                    self._wait_next_round(interval_minutes)
                    continue
                else:
                    break
    ...
    def _wait_next_round(self, minutes):
        ...
        for remaining in range(total_seconds, 0, -1):
            logging.info(f"[#status]⏳ 等待下一轮 剩余时间: {remaining // 60}分{remaining % 60}秒")
            logging.info(f"[@status] 等待下一轮{remaining / total_seconds * 100:.1f}%")
        ...
        for index, file_path in enumerate(files, 1):
            try:
                ...
                if self._is_file_locked(file_path):
                    ...
                    logging.info(f"[#file_ops]⚠️ 文件被占用: {file_name}")
                    continue
                ...
                if result:
                    ...
                    self._update_status()
                    logging.info(f"[#file_ops]✅ 处理完成: {file_name}")
                else:
                    reason = skipped_files.get(file_path, "未知原因")
                    if file_path in skipped_files:
                        logging.info(f"[#file_ops]⚠️ 跳过文件: {file_name} - {reason}")
                        self.skipped_files += 1  # 增加跳过文件计数
                    else:
                        logging.info(f"[#file_ops]⚠️ 跳过文件: {file_name} - 处理失败或不需要处理")
                    
            except Exception as e:
                logging.info(f"[#file_ops]❌ 处理文件出错 {file_path}: {e}")
    ...
    def _get_files_to_process(self, directories, processed_files, skipped_files, occupied_files):
        ...
        if files_to_process:
            logging.info(f"[#file_ops]📝 找到 {len(files_to_process)} 个待处理文件")
...
class InputHandler:
    ...
    def get_paths_from_clipboard(self):
        ...
        try:
            ...
            if valid_paths:
                logging.info(f"[#file_ops]从剪贴板读取到 {len(valid_paths)} 个有效路径")
            else:
                logging.info(f"[#file_ops]剪贴板中没有有效路径")
            return valid_paths
        except ImportError:
            logging.info(f"[#file_ops]未安装 pyperclip 模块，无法读取剪贴板")
            return []
        except Exception as e:
            logging.info(f"[#file_ops]读取剪贴板时出错: {e}")
            return []
...
class FileWatcher:
    ...
    def on_created(self, event):
        ...
        with self.processing_queue.processing_lock:
            ...
            logging.info(f"[#file_ops]添加文件到待处理列表: {file_path}")
            ...
            for file_path in self.processing_queue.pending_files:
                try:
                    ...
                    try:
                        ...
                        logging.info(f"[#file_ops]文件被占用，跳过: {file_path}")
                        files_to_remove.add(file_path)
                        continue
                    ...
                    logging.info(f"[#file_ops]检查文件时出错 {file_path}: {e}")
                    files_to_remove.add(file_path)
    ...
    def _process_file(self, file_path):
        ...
        try:
            ...
            logging.info(f"[#file_ops]处理文件时出错 {file_path}: {e}")
        finally:
            with self.processing_queue.processing_lock:
...
class ErrorHandler:
    ...
    def handle_file_error(e, file_path, operation):
        ...
        if isinstance(e, PermissionError):
            logging.info(f"[#file_ops]{operation}失败(权限不足): {file_path}")
        elif isinstance(e, FileNotFoundError):
            logging.info(f"[#file_ops]{operation}失败(文件不存在): {file_path}")
        else:
            logging.info(f"[#file_ops]{operation}失败: {file_path} - {str(e)}")

    @staticmethod
    def handle_archive_error(e, archive_path):
        """处理压缩包错误"""
        if isinstance(e, zipfile.BadZipFile):
            logging.info(f"[#file_ops]无效的压缩包格式: {archive_path}")
        elif isinstance(e, PermissionError):
            logging.info(f"[#file_ops]压缩包访问权限不足: {archive_path}")
        else:
            logging.info(f"[#file_ops]处理压缩包时出错: {archive_path} - {str(e)}")

    @staticmethod
    def handle_image_error(e, image_path):
        """处理图片错误"""
        if isinstance(e, Image.DecompressionBombError):
            logging.info(f"[#file_ops]图片过大: {image_path}")
        elif isinstance(e, Image.UnidentifiedImageError):
            logging.info(f"[#file_ops]无法识别的图片格式: {image_path}")
        else:
            logging.info(f"[#file_ops]处理图片时出错: {image_path} - {str(e)}")
...
class ConfigManager:
    ...
    def load_config(self, config_file):
        ...
        try:
            ...
            logging.info(f"[#file_ops]加载配置文件失败: {e}")
            return False

    def save_config(self):
        """保存配置到文件"""
        if not self.config_file:
            logging.info(f"[#file_ops]未指定配置文件路径")
            return False
        try:
            ...
            logging.info(f"[#file_ops]保存配置文件失败: {e}")
            return False
...
def init_performance_config(args):
    ...
    if args.performance_config:
        ...
        if not performance_config.load_config():
            logging.info(f"[#file_ops]使用默认性能配置")

def init_keywords(args):
    """初始化关键词设置"""
    if args.keywords:
        keywords = 'internal'
        logging.info(f"[#file_ops]将使用内置关键词列表: {INCLUDED_KEYWORDS}")
    else:
        keywords = None
        logging.info("[#file_ops]未启用关键词过滤")
    return keywords
...
def configure_image_conversion(args):
    ...
    if args.format == 'jxl' and args.jxl_jpeg_lossless:
        ...
        logging.info("[#file_ops]已启用 CJXL 的 JPEG 无损转换模式")
    elif args.lossless:
...
def configure_lossless_mode():
    ...
    logging.info("[#file_ops]已启用普通无损压缩模式")
    ...
    if not directories:
        ...
        for i, line in enumerate(directories):
            ...
            if os.path.exists(directory):
                ...
                logging.info(f"[#file_ops]✅ 已添加路径: {directory}")
            else:
                logging.info(f"[#file_ops]路径不存在: {directory}")
    return directories
...
def run_with_args(args):
    ...
    if directories:
        ...
        logging.info("[#file_ops]🔧 已启动性能配置调整器")
        
        logging.info(f"[#file_ops]🚀 启动{('无限循环' if args.infinite else '自动运行')}模式，每 {args.interval} 分钟运行一次...")
        monitor = Monitor()
